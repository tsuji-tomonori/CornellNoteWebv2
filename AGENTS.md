...
- 文書生成のプロジェクト入口は `uv run python tools/design.py`。`docs/generated/`にMarkdownのみを保存し、要件JSONはメモリ上で扱う。固定されたDev Standardの配布ツールは変更しない。CIは再生成・検証成功後にMarkdown差分のみdevへ反映する。
