...
sequenceDiagram
    ...
    A->>D: SELECT note_tasks, notes / 本人のノートに紐づくタスクを横断取得する
    D-->>A: 行データ（0件以上）
    break 正常終了
    A->>D: COMMIT
    D-->>A: 確定完了
    A-->>C: HTTP 200 / application/json: TaskOverview[]
    end
...
| --- | --- | --- | --- | --- |
| note_tasks, notes | SELECT | 本人のノートに紐づくタスクを横断取得する | WHERE n.owner_id = %(owner_id)s | owner_id |
